...
日历时间与 Unix 世纪秒（自 1970-01-01 00:00:00 UTC 起的秒数）相互转换，零动态分配，无 OS 依赖。支持年份范围 1970-2099。
...
- 所有函数均对 `NULL` 参数做安全处理
- 返回 `ELIB_FT_ERR_INVALID_PARAM` 表示参数越界（年份超出 1970-2099 等）
- 不考虑闰秒，始终 UTC 时区
- `epoch` 使用 `uint64_t`，可覆盖到约公元 292,277,026,596 年